(function(){
  const DIAS=[['d1','Dia 1'],['d2','Dia 2'],['d3','Dia 3'],['d4','Dia 4'],['d5','Dia 5'],['d6','Dia 6'],['d7','Dia 7']];
  ...
  function ativo(d){return d==='d1'||!!document.getElementById(`cma-jd-${d}-ativo`)?.checked;}
  function dadosDia(d){
  ...
  function copiarDia1(){
    ...
    const base={};campos.forEach(c=>base[c]=document.getElementById(`cma-jd-d1-${c}`)?.value||'');
    ['d2','d3','d4','d5','d6','d7'].forEach((d,i)=>{
      const ck=document.getElementById(`cma-jd-${d}-ativo`);if(ck)ck.checked=i<4;
      campos.forEach(c=>{const e=document.getElementById(`cma-jd-${d}-${c}`);if(e)e.value=base[c];});
    });
  }
  function mostrarDiasExtras(mostrar){
    const box=document.getElementById('cma-jd-dias-extras');if(box)box.hidden=!mostrar;
    if(mostrar)copiarDia1();
    recalcular();
  }
  function recalcular(){
    const escala12=sim('12x36');
    const dias=DIAS.map(([id,nome])=>({id,nome,d:dadosDia(id)})).filter(x=>x.d);
    let total=0,excessoDiario=0,problemasIntervalo=0,ap='';
    dias.forEach(x=>{
      ...
      const el=document.getElementById(`cma-jd-${x.id}-total`);if(el)el.textContent=fmt(x.d.trabalhado);
    });
    DIAS.forEach(([id])=>{if(!dias.some(x=>x.id===id)){const el=document.getElementById(`cma-jd-${id}-total`);if(el)el.textContent='';}});
    const excessoSemanal=escala12?0:Math.max(0,total-2640);
    document.getElementById('cma-jd-total').textContent=fmt(total);
    document.getElementById('cma-jd-resumo-sub').textContent=escala12?`${dias.length} jornada(s) informada(s) • modo 12x36`:`${dias.length} dia(s) informado(s) • referência semanal de 44h`;
    document.getElementById('cma-jd-apuracao').innerHTML=ap+linha('Total semanal informado',fmt(total),'cma-jd-total-linha');
    let analise='';
  ...
  function toggle(id,inicial,aoMudar){
    const wrap=document.querySelector(`#calculadora-jornada [data-jd-toggle="${id}"]`);if(!wrap)return;
    const bs=[...wrap.querySelectorAll('button')];
    function set(v){bs.forEach(b=>b.classList.toggle('ativo',b.dataset.valor===(v?'sim':'nao')));aoMudar?.(v);recalcular();}
    bs.forEach(b=>b.addEventListener('click',()=>set(b.dataset.valor==='sim')));set(inicial);
  }
  function linhaDia(id,nome,principal=false){
    return `<div class="cma-jd-dia" data-dia="${id}"><div class="cma-jd-dia-topo">${principal?`<strong>${nome}</strong>`:`<label class="cma-jd-check"><input id="cma-jd-${id}-ativo" type="checkbox"> <strong>${nome}</strong></label>`}<span class="cma-jd-dia-total" id="cma-jd-${id}-total"></span></div><div class="cma-jd-dia-grid"><div><label>Entrada</label><input id="cma-jd-${id}-entrada" type="time" value="08:00"></div><div><label>Saída</label><input id="cma-jd-${id}-saida" type="time" value="17:00"></div><div><label>Intervalo início</label><input id="cma-jd-${id}-int-inicio" type="time" value="12:00"></div><div><label>Intervalo fim</label><input id="cma-jd-${id}-int-fim" type="time" value="13:00"></div></div></div>`;
  }
  function criar(){
    ...
    s.innerHTML=`<div class="flex items-start justify-between gap-4 border-b border-gray-200 pb-4 mb-4"><div><h3 class="text-2xl font-bold text-blue-950">Calculadora de Jornada e Escala</h3><p class="text-sm text-gray-500 mt-1">Informe a jornada-base e, quando necessário, replique para montar a semana.</p></div></div>
      <div class="cma-jd-grid"><div class="cma-jd-card cma-jd-form"><h4>1. Jornada</h4>
        ...
        ${linhaDia('d1','Dia 1',true)}
        <div class="cma-jd-toggle" data-jd-toggle="replicar"><div class="cma-jd-cab"><div><strong>Replicar o Dia 1?</strong><small>Ao marcar Sim, os demais dias serão exibidos e poderão ser alterados.</small></div><div><button type="button" data-valor="sim">Sim</button><button type="button" data-valor="nao">Não</button></div></div></div>
        <div id="cma-jd-dias-extras" hidden>${linhaDia('d2','Dia 2')}${linhaDia('d3','Dia 3')}${linhaDia('d4','Dia 4')}${linhaDia('d5','Dia 5')}${linhaDia('d6','Dia 6')}${linhaDia('d7','Dia 7')}</div>
    ...
    const st=document.createElement('style');st.id='cma-jd-style';st.textContent=`
      .cma-jd-grid{display:grid;grid-template-columns:minmax(0,1.18fr) minmax(0,.82fr);gap:20px;align-items:start}
      .cma-jd-card{background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:18px;box-shadow:0 6px 18px rgba(15,23,42,.05)}
      .cma-jd-card h4{margin:0 0 15px;color:#172554;font-size:17px;font-weight:800}
      .cma-jd-form label{display:block;margin:8px 0 5px;color:#334155;font-size:12.5px;font-weight:700;line-height:1.25}
      .cma-jd-form input{width:100%;height:44px;padding:9px 10px;border:1px solid #cbd5e1;border-radius:9px;outline:none;background:#fff;box-sizing:border-box}
      .cma-jd-duplo{display:grid;grid-template-columns:1fr 1fr;gap:12px}
      .cma-jd-toggle{margin-top:12px;border:1px solid #e2e8f0;border-radius:11px;overflow:hidden}
      .cma-jd-cab{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:12px;background:#f8fafc}
      .cma-jd-cab>div:first-child{display:flex;flex-direction:column;gap:2px}.cma-jd-cab strong{color:#172554;font-size:13.5px}.cma-jd-cab small{color:#64748b;font-size:11px;line-height:1.4}
      .cma-jd-cab>div:last-child{display:flex;gap:5px;flex:0 0 auto}.cma-jd-cab button{min-width:48px;padding:7px 10px;border:1px solid #cbd5e1;border-radius:7px;background:#fff;color:#64748b;font-size:12px;font-weight:800}
      .cma-jd-cab button.ativo[data-valor=sim]{background:#172554;color:#fff;border-color:#172554}.cma-jd-cab button.ativo[data-valor=nao]{background:#e2e8f0;color:#334155}
      .cma-jd-dia{margin-top:12px;padding:13px;border:1px solid #e2e8f0;border-radius:11px;background:#fff}.cma-jd-dia-topo{display:flex;align-items:center;justify-content:space-between;min-height:22px;color:#172554;font-size:13px}.cma-jd-check{display:flex!important;align-items:center;gap:7px;margin:0!important}.cma-jd-check input{width:auto!important;height:auto!important}.cma-jd-dia-total{font-size:11.5px;color:#64748b;white-space:nowrap}
      .cma-jd-dia-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;align-items:end}.cma-jd-dia-grid>div{min-width:0}
      .cma-jd-resultados{display:flex;flex-direction:column;gap:14px;min-width:0}.cma-jd-resumo{padding:22px;border-radius:14px;background:linear-gradient(135deg,#061a46,#082f7d);color:#fff}.cma-jd-resumo span{display:block;color:#bfdbfe;font-size:13px}.cma-jd-resumo strong{display:block;margin-top:5px;font-size:34px}.cma-jd-resumo em{display:block;margin-top:7px;color:#fbbf24;font-size:12px;font-style:normal;font-weight:700}
      .cma-jd-linha{display:flex;justify-content:space-between;gap:18px;padding:8px 0;border-bottom:1px solid #f1f5f9;color:#475569;font-size:13px}.cma-jd-linha strong{color:#1e293b;text-align:right}.cma-jd-total-linha{margin-top:4px;padding-top:11px;border-top:2px solid #dbeafe;font-weight:800}.cma-jd-status-lista{display:flex;flex-direction:column;gap:8px}.cma-jd-status{padding:10px 12px;border-radius:9px;font-size:12.5px;font-weight:700;line-height:1.4}.cma-jd-status.ok{background:#ecfdf5;color:#065f46;border:1px solid #a7f3d0}.cma-jd-status.atencao{background:#fffbeb;color:#92400e;border:1px solid #fde68a}.cma-jd-status.erro{background:#fef2f2;color:#991b1b;border:1px solid #fecaca}.cma-jd-como summary{cursor:pointer;color:#172554;font-weight:800}.cma-jd-memoria{margin-top:12px}.cma-jd-nota{margin-top:12px;color:#64748b;font-size:11.5px;line-height:1.5}
      @media(max-width:1100px){.cma-jd-dia-grid{grid-template-columns:1fr 1fr}}
      @media(max-width:900px){.cma-jd-grid{grid-template-columns:1fr}.cma-jd-dia-grid{grid-template-columns:repeat(4,minmax(0,1fr))}}
      @media(max-width:640px){.cma-jd-duplo,.cma-jd-dia-grid{grid-template-columns:1fr 1fr}.cma-jd-cab{align-items:flex-start;flex-direction:column}.cma-jd-cab>div:last-child{width:100%}.cma-jd-cab button{flex:1}.cma-jd-form input{font-size:16px}.cma-jd-resumo strong{font-size:30px}}
    `;document.head.appendChild(st);
    s.querySelectorAll('input').forEach(i=>{i.addEventListener('input',recalcular);i.addEventListener('change',recalcular);});
    ['cma-jd-d1-entrada','cma-jd-d1-saida','cma-jd-d1-int-inicio','cma-jd-d1-int-fim'].forEach(id=>document.getElementById(id)?.addEventListener('change',()=>{if(sim('replicar'))copiarDia1();recalcular();}));
    toggle('12x36',false,v=>{const t=document.getElementById('cma-jd-12-status');if(t)t.textContent=v?'Modo 12x36 ativado':'Modo comum';});
    toggle('replicar',false,mostrarDiasExtras);
    recalcular();
    if(window.CMAFerramentas)CMAFerramentas.adicionarAcoes('calculadora-jornada',{onLimpar:()=>{mostrarDiasExtras(false);recalcular();}});
