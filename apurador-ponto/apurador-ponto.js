(function(){
  ...
  function analisarDia({previsto=0,marcacoes=[],situacao='trabalhado',especial=false,margem=10,noturno=false}={}){
  ...
  function recalcular(){
    const margem=Math.max(0,Number(document.getElementById('cma-ap-margem')?.value)||0),noturno=document.getElementById('cma-ap-noturno')?.checked||false,hoje=new Date();hoje.setHours(0,0,0,0);
    ...
    document.querySelectorAll('#cma-ap-corpo tr').forEach(tr=>{
      ...
      const r=analisarDia({previsto:minutos(get('previsto').value)||0,marcacoes:['m1','m2','m3','m4'].map(n=>get(n).value),situacao:get('situacao').value,especial,margem,noturno});
      tot.previsto+=r.previsto;
      if(r.valido){tot.informados++;tot.trabalhado+=r.trabalhado+r.credito;tot.atraso+=r.atraso;tot.saldo+=r.saldo;tot.especial+=r.extra100||0;tot.comum+=r.extra50||0;tot.noturno+=r.noturnoReduzido||0;if(r.margem)tot.margem++;}
      ...
      tr.querySelector('[data-saida="noturno"]').textContent=r.valido&&r.noturnoReduzido?formatar(r.noturnoReduzido):'—';
  ...
  function alternarNoturno(){const sec=document.getElementById('apurador-ponto');sec?.classList.toggle('cma-ap-noturno-ativo',document.getElementById('cma-ap-noturno')?.checked||false);recalcular();}
  function configurarNoturno(){
    const input=document.getElementById('cma-ap-noturno'),botoes=[...document.querySelectorAll('#apurador-ponto [data-ap-noturno]')];if(!input)return;
    function definir(ativo){input.checked=ativo;botoes.forEach(b=>{const selecionado=(b.dataset.apNoturno==='sim')===ativo;b.classList.toggle('ativo',selecionado);b.setAttribute('aria-pressed',String(selecionado));});alternarNoturno();}
    botoes.forEach(b=>b.addEventListener('click',()=>definir(b.dataset.apNoturno==='sim')));definir(false);
  }
  function criar(){
    ...
    const s=document.createElement('section');s.id='apurador-ponto';s.className='manual-section hidden fade-in cma-calculadora-padrao';s.innerHTML=`
      ...
      <div class="cma-ap-noturno-opcao"><input id="cma-ap-noturno" type="checkbox" hidden><span><strong>Há trabalho no período noturno?</strong><small>Ao selecionar “Sim”, serão apuradas as horas urbanas entre 22h e 5h, considerando a hora reduzida de 52min30s.</small></span><div class="cma-ap-noturno-escolha" role="group" aria-label="Apurar adicional noturno"><button type="button" data-ap-noturno="sim" aria-pressed="false">Sim</button><button type="button" data-ap-noturno="nao" class="ativo" aria-pressed="true">Não</button></div></div>
      ...
      <div class="cma-ap-resumo"><div><span>Horas trabalhadas/abonadas</span><strong id="cma-ap-trabalhado">00:00</strong></div><div class="principal"><span>Horas extras 50%</span><strong id="cma-ap-comum">00:00</strong></div><div class="principal cma-ap-100"><span>Horas extras 100%</span><strong id="cma-ap-especial">00:00</strong></div><div class="cma-ap-col-noturno"><span>Horas noturnas reduzidas</span><strong id="cma-ap-noturno-total">00:00</strong></div><div><span>Atrasos e faltas</span><strong id="cma-ap-atraso">00:00</strong></div></div>
  ...
  window.CMAApuradorPonto={minutos,duracao,formatar,calcularNoturno,analisarDia,criar,versao:'1.4'};
