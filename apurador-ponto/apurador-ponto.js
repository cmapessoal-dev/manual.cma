(function(){
  ...
  function aplicarJornadaSemanal(){
    ...
    document.querySelectorAll('#cma-ap-corpo tr').forEach(tr=>{
      const campo=tr.querySelector('[data-campo="previsto"]');if(campo)campo.value=formatar(previstoDoDia(dataLocal(tr.dataset.apDia).getDay()));
    });
    recalcular();
  }
  function recalcular(){
    const margem=Math.max(0,Number(document.getElementById('cma-ap-margem')?.value)||0),hoje=new Date();hoje.setHours(0,0,0,0);
    const tot={previsto:0,trabalhado:0,comum:0,especial:0,atraso:0,saldo:0,pendentes:0,naoInformados:0,informados:0,margem:0};
    document.querySelectorAll('#cma-ap-corpo tr').forEach(tr=>{
      ...
      if(r.valido){tot.informados++;tot.trabalhado+=r.trabalhado+r.credito;tot.atraso+=r.atraso;tot.saldo+=r.saldo;if(especial)tot.especial+=r.extra;else tot.comum+=r.extra;if(r.margem)tot.margem++;}
  ...
  window.CMAApuradorPonto={minutos,duracao,formatar,analisarDia,criar,versao:'1.1'};
