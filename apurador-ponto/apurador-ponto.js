(function(){
  ...
  function analisarDia({previsto=0,marcacoes=[],situacao='trabalhado',especial=false,margem=10}={}){
    const base=Math.max(0,Number(previsto)||0);
    if(situacao==='folga'||situacao==='suspensao')return {valido:true,previsto:0,trabalhado:0,credito:0,extra:0,atraso:0,saldo:0,especial,margem:false};
    if(situacao==='abono')return {valido:true,previsto:base,trabalhado:0,credito:base,extra:0,atraso:0,saldo:0,especial,margem:false};
    if(situacao==='falta')return {valido:true,previsto:base,trabalhado:0,credito:0,extra:0,atraso:base,saldo:-base,especial,margem:false};
    ...
    const trabalhado=pares.reduce((a,b)=>a+b,0),saldo=trabalhado-base;
    return {valido:true,previsto:base,trabalhado,credito:0,extra:Math.max(0,saldo),atraso:Math.max(0,-saldo),saldo,especial,margem:Math.abs(saldo)>0&&Math.abs(saldo)<=Math.max(0,Number(margem)||0)};
  }
  ...
  function previstoDoDia(dia){return PADRAO[dia]||0;}
  ...
  function linhaData(data){
    ...
    return `<tr data-ap-dia="${chave}"><td class="cma-ap-data"><strong>${String(data.getDate()).padStart(2,'0')}</strong><span>${DIAS[data.getDay()]}${feriado?` • ${escapar(feriado)}`:''}</span></td><td><select data-campo="situacao" aria-label="Situação em ${chave}">${opcoesSituacao('trabalhado')}</select></td><td><input data-campo="previsto" type="time" value="${formatar(previstoDoDia(data.getDay()))}" aria-label="Jornada prevista em ${chave}"></td><td class="cma-ap-marcas"><input data-campo="m1" type="time" aria-label="Primeira marcação em ${chave}"><input data-campo="m2" type="time" aria-label="Segunda marcação em ${chave}"><input data-campo="m3" type="time" aria-label="Terceira marcação em ${chave}"><input data-campo="m4" type="time" aria-label="Quarta marcação em ${chave}"></td><td data-saida="trabalhado">—</td><td data-saida="saldo">—</td><td data-saida="status"><span class="cma-ap-tag ${especial?'especial':'pendente'}">${especial?'Dom./feriado':'Pendente'}</span></td></tr>`;
  }
  ...
  function adicionarFeriado(){
    ...
    if(!data||!nome)return;estado.feriados.set(data,nome);renderFeriados();montarPeriodo();
  }
  function renderFeriados(){
    ...
    box.querySelectorAll('[data-remover-feriado]').forEach(b=>b.addEventListener('click',()=>{estado.feriados.delete(b.dataset.removerFeriado);renderFeriados();montarPeriodo();}));
  }
  function recalcular(){
    const margem=Math.max(0,Number(document.getElementById('cma-ap-margem')?.value)||0),tot={previsto:0,trabalhado:0,comum:0,especial:0,atraso:0,saldo:0,pendentes:0,margem:0};
    document.querySelectorAll('#cma-ap-corpo tr').forEach(tr=>{
      ...
      const r=analisarDia({previsto:minutos(get('previsto').value)||0,marcacoes:['m1','m2','m3','m4'].map(n=>get(n).value),situacao:get('situacao').value,especial,margem});
      tot.previsto+=r.previsto;
      if(r.valido){tot.trabalhado+=r.trabalhado+r.credito;tot.atraso+=r.atraso;tot.saldo+=r.saldo;if(especial)tot.especial+=r.extra;else tot.comum+=r.extra;if(r.margem)tot.margem++;}
      else if(!r.vazio)tot.pendentes++;
      tr.querySelector('[data-saida="trabalhado"]').textContent=r.valido?formatar(r.trabalhado+r.credito):'—';
      const saldo=tr.querySelector('[data-saida="saldo"]');saldo.textContent=r.valido?formatar(r.saldo,true):'—';saldo.className=r.saldo>0?'positivo':r.saldo<0?'negativo':'';
      const status=tr.querySelector('[data-saida="status"]');let texto='Pendente',classe='pendente';
      if(r.valido){if(r.margem){texto='Na margem';classe='margem';}else if(r.saldo>0){texto=especial?'Excedente especial':'Excedente';classe='positivo';}else if(r.saldo<0){texto='Atraso/falta';classe='negativo';}else{texto='Conferido';classe='ok';}}
    ...
    const valores={previsto:tot.previsto,trabalhado:tot.trabalhado,comum:tot.comum,especial:tot.especial,atraso:tot.atraso,saldo:tot.saldo};
    Object.entries(valores).forEach(([k,v])=>{const e=document.getElementById(`cma-ap-${k}`);if(e)e.textContent=formatar(v,k==='saldo');});
    const aviso=document.getElementById('cma-ap-aviso');
    aviso.textContent=tot.pendentes?`${tot.pendentes} dia(s) com marcações incompletas foram excluídos da apuração.`:tot.margem?`${tot.margem} dia(s) ficaram dentro da margem informada. Os minutos continuam demonstrados e não foram descontados automaticamente.`:'Preencha as marcações para iniciar a conferência.';
    aviso.className=`cma-ap-aviso ${tot.pendentes?'erro':tot.margem?'atencao':''}`;
  }
  function limpar(){
  ...
  function criar(){
    ...
    const s=document.createElement('section');s.id='apurador-ponto';s.className='manual-section hidden fade-in cma-calculadora-padrao';s.innerHTML=`
      ...
      <div class="cma-ap-resumo"><div><span>Previsto</span><strong id="cma-ap-previsto">00:00</strong></div><div><span>Trabalhado/abonado</span><strong id="cma-ap-trabalhado">00:00</strong></div><div><span>Excedente comum</span><strong id="cma-ap-comum">00:00</strong></div><div><span>Excedente dom./feriado</span><strong id="cma-ap-especial">00:00</strong></div><div><span>Atrasos/faltas</span><strong id="cma-ap-atraso">00:00</strong></div><div class="principal"><span>Saldo bruto</span><strong id="cma-ap-saldo">00:00</strong></div></div>
      <div id="cma-ap-aviso" class="cma-ap-aviso">Preencha as marcações para iniciar a conferência.</div>
      <div class="cma-ap-tabela-wrap"><table class="cma-ap-tabela"><caption>Apuração de <span id="cma-ap-mes"></span></caption><thead><tr><th>Dia</th><th>Situação</th><th>Previsto</th><th>Marcações (entrada e saída)</th><th>Apurado</th><th>Saldo</th><th>Conferência</th></tr></thead><tbody id="cma-ap-corpo"></tbody></table></div>
      <div class="cma-ap-nota"><strong>Como interpretar</strong><p>O resultado é uma conferência bruta das marcações. A margem informada apenas sinaliza diferenças pequenas: nenhum minuto é retirado automaticamente. Banco de horas, compensações, percentuais de horas extras, adicional noturno, regras coletivas e jornadas especiais devem ser analisados antes do fechamento.</p><p>Os dados permanecem somente nesta página e não são enviados ou salvos pelo Manual.</p></div>`;
    const base=document.getElementById('baselegal');if(base)main.insertBefore(s,base);else main.appendChild(s);instalarEstilo();
    document.getElementById('cma-ap-periodo').addEventListener('change',montarPeriodo);document.getElementById('cma-ap-margem').addEventListener('input',recalcular);document.getElementById('cma-ap-add-feriado').addEventListener('click',adicionarFeriado);document.getElementById('cma-ap-limpar').addEventListener('click',limpar);document.getElementById('cma-ap-imprimir').addEventListener('click',imprimir);renderFeriados();montarPeriodo();
  ...
  window.CMAApuradorPonto={minutos,duracao,formatar,analisarDia,criar,versao:'1.0'};
