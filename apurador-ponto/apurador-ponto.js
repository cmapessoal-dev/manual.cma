(function(){
  ...
  function analisarDia({previsto=0,marcacoes=[],situacao='trabalhado',especial=false,margem=10}={}){
    ...
    const trabalhado=pares.reduce((a,b)=>a+b,0),saldo=trabalhado-base,extra=Math.max(0,saldo);
    return {valido:true,previsto:base,trabalhado,credito:0,extra,extra50:especial?0:extra,extra100:especial?trabalhado:0,atraso:Math.max(0,-saldo),saldo,especial,margem:!especial&&Math.abs(saldo)>0&&Math.abs(saldo)<=Math.max(0,Number(margem)||0)};
  }
  ...
  function recalcular(){
    ...
    document.querySelectorAll('#cma-ap-corpo tr').forEach(tr=>{
      ...
      if(r.valido){tot.informados++;tot.trabalhado+=r.trabalhado+r.credito;tot.atraso+=r.atraso;tot.saldo+=r.saldo;tot.especial+=r.extra100||0;tot.comum+=r.extra50||0;if(r.margem)tot.margem++;}
    ...
    const valores={trabalhado:tot.trabalhado,comum:tot.comum,especial:tot.especial,atraso:tot.atraso};
  ...
  function criar(){
    ...
    const largura=()=>document.getElementById('manual-conteudo')?.classList.toggle('cma-apurador-aberto',s.classList.contains('active')&&!s.classList.contains('hidden'));
    new MutationObserver(largura).observe(s,{attributes:true,attributeFilter:['class']});largura();
  ...
  window.CMAApuradorPonto={minutos,duracao,formatar,analisarDia,criar,versao:'1.2'};
