(function(){
  ...
  function recalcular(){
    ...
    [['cma-res-dias-saldo',automaticos.diasSaldo],['cma-res-avos-ferias',automaticos.avosFerias],['cma-res-avos-decimo',automaticos.avosDecimo]].forEach(([id,valor])=>{const el=document.getElementById(id);if(el)el.value=String(valor);});
  ...
  function criar(){
    ...
    const secao=document.createElement('section');secao.id='calculadora-rescisao';secao.className='manual-section hidden fade-in cma-calculadora-padrao';secao.innerHTML=`
      ...
      <div class="cma-res-intro"><strong>Apuração automática por datas</strong><p>O simulador calcula o saldo de salário, os avos de 13º e os avos de férias proporcionais. Nesta etapa, apenas férias vencidas e dias de aviso permanecem informados manualmente.</p></div>
      <div class="cma-res-grid"><div class="cma-res-card cma-res-form"><h4>1. Dados para o cálculo</h4>
        ...
        <div class="cma-res-duplo"><div><label>Períodos de férias vencidas</label><input id="cma-res-ferias-vencidas" type="number" min="0" max="10" step="1" value="0"></div><div><label>Dias de aviso-prévio</label><input id="cma-res-dias-aviso" type="number" min="0" max="90" step="1" value="30"></div></div>
      </div>
  ...
  window.CMACalculadoraRescisao={calcularBase,calcularAutomaticos,contarAvosDecimo,contarAvosFerias,recalcular,criar,versao:'0.2'};
