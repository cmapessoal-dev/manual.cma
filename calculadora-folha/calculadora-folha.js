(function(){
  ...
  function recalcular(){
    ...
    const remuneracaoINSS=Math.max(0,tributavelAntes-descontosJornada);
    const rendimentosIRRF=Math.max(0,remuneracaoINSS+premio);
    ...
    const inss=T().calcularINSS(remuneracaoINSS);
    const irrf=T().calcularIRRF({rendimentos:rendimentosIRRF,inss:inss.valor,dependentes,pensao});
    ...
    document.getElementById('cma-folha-rem-trib').textContent=moeda(remuneracaoINSS);
    const irRend=document.getElementById('cma-folha-ir-rendimentos');if(irRend)irRend.textContent=moeda(rendimentosIRRF);
  ...
  function criar(){
    ...
    const s=document.createElement('section');s.id='calculadora-folha';s.className='manual-section hidden fade-in';s.innerHTML=`
      ...
      <div id="exp-calculadora-folha" class="hidden bg-slate-100 border border-slate-300 p-3 rounded mb-4 text-xs text-slate-700 space-y-2"><p><strong>Remuneração para INSS:</strong> salário e demais parcelas previdenciárias, acrescidos das variáveis e reduzidos pelos descontos de faltas, atrasos e DSR.</p><p><strong>INSS:</strong> cálculo progressivo conforme as faixas vigentes em 2026, respeitando o teto previdenciário.</p><p><strong>IRRF:</strong> a ferramenta compara deduções legais com o desconto simplificado mensal e utiliza automaticamente a alternativa mais favorável, aplicando também a redução do imposto vigente em 2026.</p><p><strong>Prêmio:</strong> quando corretamente caracterizado para fins trabalhistas/previdenciários, pode não integrar a base do INSS; para esta simulação, o valor continua compondo os rendimentos tributáveis do IRRF.</p><p><strong>Horas extras:</strong> calculadas sobre a base da hora informada e com DSR automático pela competência. O adicional noturno deve ser informado em horas noturnas já apuradas/convertidas; para jornadas complexas, utilize a calculadora específica de Adicional Noturno.</p></div>
      ...
      </div><div class="cma-folha-resultados"><div class="cma-folha-resumo"><span>Salário líquido estimado</span><strong id="cma-folha-liquido">R$ 0,00</strong><em id="cma-folha-resumo-sub">Bruto: R$ 0,00 • INSS: R$ 0,00 • IRRF: R$ 0,00</em></div><div class="cma-folha-card"><h4>2. Proventos</h4><div id="cma-folha-proventos"></div></div><div class="cma-folha-card"><h4>3. Descontos e líquido</h4><div id="cma-folha-descontos"></div></div><div class="cma-folha-card cma-folha-memoria"><h4>4. Memória tributária</h4><div><span>Base da hora</span><strong id="cma-folha-base-hora">R$ 0,00</strong></div><div><span>Valor da hora normal</span><strong id="cma-folha-valor-hora">R$ 0,00</strong></div><div><span>Remuneração para INSS</span><strong id="cma-folha-rem-trib">R$ 0,00</strong></div><div><span>Rendimentos tributáveis do IRRF</span><strong id="cma-folha-ir-rendimentos">R$ 0,00</strong></div><div><span>Base limitada do INSS</span><strong id="cma-folha-base-inss">R$ 0,00</strong></div><div><span>INSS do empregado</span><strong id="cma-folha-inss">R$ 0,00</strong></div><div><span>Método do IRRF</span><strong id="cma-folha-ir-metodo">—</strong></div><div><span>Dedução usada no IRRF</span><strong id="cma-folha-ir-deducao">R$ 0,00</strong></div><div><span>Base do IRRF</span><strong id="cma-folha-ir-base">R$ 0,00</strong></div><div><span>IRRF antes da redução 2026</span><strong id="cma-folha-ir-antes">R$ 0,00</strong></div><div><span>Redução do IRRF 2026</span><strong id="cma-folha-ir-reducao">R$ 0,00</strong></div><div><span>Fórmula DSR das horas extras</span><strong id="cma-folha-dsr-formula">—</strong></div></div></div></div>
      <div class="bg-amber-50 border-l-4 border-amber-600 p-4 rounded-r shadow-sm mt-4"><strong class="text-amber-900 block mb-1">Simulação para conferência</strong><p class="text-amber-950 text-sm leading-relaxed">A ferramenta estima uma folha mensal comum. Incidências podem variar conforme natureza da rubrica, convenção coletiva, múltiplos vínculos, decisões judiciais e particularidades do empregado. O campo de prêmio pressupõe verba corretamente caracterizada para não incidência previdenciária; o IRRF continua sendo calculado sobre o rendimento.</p></div>`;
